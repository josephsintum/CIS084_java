...
// Version:      1.0
// Inputs:       itemID
// Output:       shoppingCart, shoppingCartCount, total
...
public class BooksAndClothesPolymorphism {
    ...
    private static final Item[] ITEM_LIST = {
            ...
            new Pants(3001, "Jeans", "Levi's", 34, 34, "Medium Stonewash", 59.50, 12),
            new Pants(3002, "Chinos", "Bonobos", 36, 34, "Black", 95.20, 22),
            new Pants(3003, "Joggers", "Lululemon", 32, 34, "Black", 118.00, 5),
            new Pants(3004, "Khaki", "J-Crew", 34, 34, "Slate", 34.95, 15),
    };
    private static final Item[] shoppingCart = new Item[10];   // contains items in user's shopping cart
    ...
    public static void main(String[] args) {
        ...
        do {
            try {
                ...
                else if (ITEM_LIST[itemIndex].getInStock() <= 0) {          // The item was found
                    System.out.println("Out of Stock. Please try again later");
                } else if (shoppingCartCount < shoppingCart.length) {       // The item is in stock
                    shoppingCart[shoppingCartCount] = ITEM_LIST[itemIndex];
                    ITEM_LIST[itemIndex].setInStock(ITEM_LIST[itemIndex].getInStock() - 1);
                    shoppingCartCount++;  // keep track of items in the cart
                } else {
                    System.out.println("Your shopping cart is full");
                    itemSelected = 0;       // exit loop when cart full
                }
            } catch (InputMismatchException | StringIndexOutOfBoundsException e) {
                System.out.println("Illegal selection. Try again");
                stdin.next();       // clear the illegal input
                itemSelected = 1;   // continue the loop
            }
